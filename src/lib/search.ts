...
export function google(query: string): Promise<SearchResult[]> {
	const { urlTemplate, regex } = SEARCH_ENGINES.Google;
	return _search(query, urlTemplate, regex, 'Google');
}

export function duckduckgo(query: string): Promise<SearchResult[]> {
	const { urlTemplate, regex } = SEARCH_ENGINES.DuckDuckGo;
	return _search(query, urlTemplate, regex, 'DuckDuckGo');
}

export function yandex(query: string): Promise<SearchResult[]> {
	const { urlTemplate, regex } = SEARCH_ENGINES.Yandex;
	return _search(query, urlTemplate, regex, 'Yandex');
}
