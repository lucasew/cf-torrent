...
## 2026-01-11 - Centralize API Response Logic
**Vulnerability:** The series stream endpoint at `src/routes/api/stremio/stream/series/[name]/+server.ts` was constructing its own JSON response, bypassing the centralized `json` helper function used by the movie stream endpoint. This inconsistency created a security risk, as any future security enhancements or sanitization added to the helper would not have been applied to the series endpoint.
**Learning:** Inconsistent error and response handling can lead to subtle but critical vulnerabilities. When a centralized helper function is available, it should be used consistently to ensure that all endpoints benefit from the same security and logging logic.
**Prevention:** Always use centralized helper functions for common tasks like creating API responses. This not only reduces code duplication but also ensures that security policies are applied uniformly across the application. Future code reviews should flag any manual response creation and enforce the use of the `json` helper.
